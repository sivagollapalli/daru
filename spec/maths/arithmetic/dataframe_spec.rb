describe Daru::DataFrame do
  let!(:df) do
    Daru::DataFrame.new(a: [1,2,3,4,5], b: ['a','e','i','o','u'], c: [10,20,30,40,50])
  end

  let!(:left) do
    Daru::DataFrame.new({ a: [1,nil,nil,4], b: [10,nil,nil,40], c: [5,6,7,8] }, index: [0,4,5,3])
  end

  let!(:right) do
    Daru::DataFrame.new({ a: [1,2,3,4,5], b: [10,20,30,40,50] }, index: [0,1,2,3,6])
  end

  context "#+" do
    context "adds a number to all numeric vectors" do
      it { expect(df + 2).to eq(Daru::DataFrame.new(a: [3, 4, 5, 6, 7], 
						    b: ['a', 'e', 'i', 'o', 'u'],
					            c: [12, 22, 32, 42, 52])) }
    end

    context "adds two dataframes to produce a third" do
      it { expect(left + right).to eq(Daru::DataFrame.new({ a: [2, nil, nil, 8, nil, nil],
  ...
  context "#-" do
    context "subtracts a number from all numeric vectors" do
      it { expect(df - 2).to eq(Daru::DataFrame.new({ a: [-1, 0, 1, 2, 3],
                                                      b: ['a','e','i','o','u'],
                                                      c: [8,18,28,38,48] })) }
    end

    context "subtracts a data frame from another" do
      it { expect(left - right).to eq(Daru::DataFrame.new({ a: [0, nil, nil, 0, nil, nil],
  ...
  context "#*" do
    context "multiplies all numeric vectors by number" do
      it { expect(df * 2).to eq(Daru::DataFrame.new(a: [2, 4, 6, 8, 10], 
						    b: ['a', 'e', 'i', 'o', 'u'],
   						    c: [20, 40, 60, 80, 100])) }
    end

    context "multipies two dataframes to produce a third" do
      it { expect(left * right).to eq(Daru::DataFrame.new({ a: [1, nil, nil, 16, nil, nil],
